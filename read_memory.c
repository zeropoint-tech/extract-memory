...
int read_maps_file(int pid, FILE *file_bin_out, FILE *file_maps_out,
  ...
  FILE *file_maps;
  int ch, maps_entries, pos_addr, pos_pathname, field, entry, i, num_heap_pages,
      num_anon_pages, num_stack_pages, num_library_pages, num_other_pages,
      num_filemapped_pages, status;
  Maps_entry *maps_arr;
  ...
  while ((ch = fgetc(file_maps)) != EOF) {
    ...
    if (field == 0) {
      ...
      if (ch == '\n') {
        ...
        maps_arr[entry].pathname[pos_pathname] = ch;
        pos_pathname++;
      }
    }
  }
  ...
  for (i = 0; i < maps_entries; i++) {
    ...
    printf("Start address: %llx, Valid: %d, Entry type: %d\n",
           maps_arr[i].start_addr, maps_arr[i].valid, maps_arr[i].entry_type);

    if (maps_arr[i].valid) {
      categorize_entry(&maps_arr[i]);
      status = wr_maps_entry_to_file(pid, &maps_arr[i], file_bin_out, type_out);
      if (status != 0) {
