...
void
categorize_entry(Maps_entry *map) {
    int str_len;
    char subbuff[6];
...
wr_maps_entry_to_file(int pid, Maps_entry *map, FILE *bin_out, FILE *type_out) {
    ...
    for (addr = map->start_addr; addr < map->end_addr; addr += PAGE_SIZE) {
        ...
        if (GET_BIT(read_val, 63)) {
            ...
            printf("<%d::%llx>\n", pid,
                   (long long unsigned int)GET_PFN(read_val));
...
void
get_present_pages(int pid, Maps_entry *map) {
    char path_buf[0x100];
...
read_maps_file(int pid, FILE *file_bin_out, FILE *file_maps_out, FILE *type_out) {
    ...
    FILE *file_maps;
    int c, maps_entries, pos_addr, pos_pathname, field, entry, i,
        num_heap_pages, num_anon_pages, num_stack_pages, num_library_pages,
